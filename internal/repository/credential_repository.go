...
type CredentialRepository interface {
	...
	UpdatePassword(ctx context.Context, tx *sql.Tx, userID, hash, salt string) error
}
...
func (r *PostgresCredentialRepository) Create(ctx context.Context, tx *sql.Tx, cred *model.UserCredential) error {
	...
	var err error
	if tx != nil {
		err = tx.QueryRowContext(ctx, query, cred.ID, cred.UserID, cred.PasswordHash, cred.PasswordSalt, cred.PasswordAlgo).
			Scan(&cred.ID, &cred.CreatedAt, &cred.UpdatedAt)
	} else {
		err = r.db.QueryRowContext(ctx, query, cred.ID, cred.UserID, cred.PasswordHash, cred.PasswordSalt, cred.PasswordAlgo).
			Scan(&cred.ID, &cred.CreatedAt, &cred.UpdatedAt)
	}
...
func (r *PostgresCredentialRepository) UpdatePassword(ctx context.Context, tx *sql.Tx, userID, hash, salt string) error {
	...
	var err error
	if tx != nil {
		_, err = tx.ExecContext(ctx, query, userID, hash, salt)
	} else {
		_, err = r.db.ExecContext(ctx, query, userID, hash, salt)
	}
